...
**同一格子内多个方案体量重叠**：逐层判定，取该层面积最大的那块。当其中一块完全
包含其余（裙房套塔楼、退台）时这就是精确并集，不告警；只有互不包含的部分重叠才
是近似，此时体积被低估并明确告警。真正做全并集需要多边形布尔（shapely 或自己
实现 Greiner–Hormann）。

早期版本按**整列**选一块，导致 18 米裙房把 54 米塔楼整个盖掉——分析的是一栋没画过
的楼，还报"无需切削"。回归测试见 `tests/test_headless_geometry.py` 的
`OverlappingDesignVolumeTests`。
