#! python 3
"""
Project source: src/rhino8/SunlightEnvelope_Rhino8_Continuous_SDK.py

Grasshopper Python 3 Script Component - SDK Mode
Rhino 8 - Longest Continuous Direct Sun
...
4. Save the script so the RunScript signature can synchronize parameters.

Rhino 8 synchronizes the 15 inputs from the RunScript signature. If a specific
Rhino 8 service release does not rename the return ports automatically, create
or rename the three outputs once, in this order: P, H, I.

Recommended parameter access
----------------------------
...
the longest uninterrupted direct-sun duration at every sample point.
...
5. Output sample points and longest continuous sun hours as matching Data Trees.
...
H : DataTree[float]
    Longest continuous direct-sun hours matching P exactly.
...
def convert_geometry_to_meshes(geometry):
    ...
    Supported inputs include Mesh, Brep, Extrusion, Surface, and SubD.
...
def calculate_layer_sun_hours(
    ...
    """
    Calculate the longest continuous direct-sun duration for one layer.
    ...
    for point in layer_points:
        current_continuous_hours = 0.0
        longest_continuous_hours = 0.0
        ...
        for sample in sun_samples:
            ...
            if point_has_direct_sun(
                ...
                current_continuous_hours += sample["duration_hours"]

                if current_continuous_hours > longest_continuous_hours:
                    longest_continuous_hours = current_continuous_hours
            else:
                current_continuous_hours = 0.0
        ...
        sun_hours.append(
            round(longest_continuous_hours, 10)
        )
        global_counter += 1
...
def calculate_all_sun_hours(
    ...
    """
    Calculate longest continuous direct-sun hours for every sample point.
    ...
    if context_mesh is None:
        current_continuous_hours = 0.0
        unobstructed_continuous_hours = 0.0
        previous_sample_end = None
        ...
        for sample in sun_samples:
            ...
            if current_continuous_hours > unobstructed_continuous_hours:
                unobstructed_continuous_hours = current_continuous_hours

            previous_sample_end = sample.get("end")

        unobstructed_continuous_hours = round(
            unobstructed_continuous_hours,
            10
        )
        ...
        for layer_points in sample_layers:
            ...
            while len(layer_hours) < layer_item_count:
                ...
                layer_hours.extend(
                    [unobstructed_continuous_hours] * chunk_size
                )
...
def calculate_result_statistics(hour_layers):
    """Calculate statistics for longest continuous sun-hour values."""
    value_count = 0
    value_sum = 0.0
    minimum = None
    maximum = None
...
def build_info(
    ...
    if hour_layers is not None:
        ...
        info.append("--- Longest Continuous Sun-Hour Results ---")
        info.append(
            "Minimum Continuous Sun Hours: {0}".format(
                format_number(minimum, 4)
            )
        )
        info.append(
            "Maximum Continuous Sun Hours: {0}".format(
                format_number(maximum, 4)
            )
        )
        info.append(
            "Average Continuous Sun Hours: {0}".format(
    ...
    info.append(
        "H: Matching Data Tree of longest continuous direct-sun hours."
    )
    info.append(
...
def execute(
    ...
    if not bool(Run):
        return (
            ...
            [
                ...
                "Python outputs points and longest continuous sun hours only."
            ]
        )
    ...
    if not sun_samples:
        all_warnings.append(
            ...
            "All continuous sun-hour values will be zero."
        )
...
class Script_Instance(Grasshopper.Kernel.GH_ScriptInstance):
    ...
    def RunScript(
        ...
        """
        Return points, longest continuous sun hours, and information.

        P and H are constructed together by build_matching_output_trees(), so
        their paths, item counts, and item order remain strictly identical.
        """
        try:
            P, H, I = execute(
                Boundary,
